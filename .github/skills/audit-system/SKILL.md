...
- [ ] **Slippage pip-size formula covers all instrument types** — `engine/order_manager.py` slippage calc. The formula `point * (10 if digits in (3,5) else 1)` is FOREX-ONLY. For stocks (digits≤2) it produces inflated pip counts (e.g. $0.67 slip = "67 pips" on a $40 stock). For crypto indices the number is meaningless. Rule: if `digits in (5,3)` → pip=point×10; if `digits≤2` OR symbol matches index/crypto keywords → pip_size=1.0 (raw price). Verify the same logic is applied consistently in `TrailingStopManager._get_pip_value()`.
- [ ] **Trailing stop config covers ALL non-forex scanner symbols** — for every symbol in `config/scanner.json` that is NOT standard forex (i.e. stocks, crypto, commodities, futures, indices), verify `config/app.json` `trailing_stops.<mode>.symbol_overrides` has an explicit `activation_price`/`trail_price` entry. Symbols without an override fall through to `activation_pips × _get_pip_value()` — for stocks with digits=2 this gives tiny $0.20 activation distance and $0.15 trail, causing immediate trailing activation and near-zero lock-in. Cross-reference the three scanner modes (scalping/day_trading/swing) against their respective `symbol_overrides` blocks.

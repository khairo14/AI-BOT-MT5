  ...
  if (!data) {
    return (
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        <p className="text-gray-500 text-sm">Failed to load scanner performance data</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
      ...
      <div className="space-y-4">
        {(Object.entries(MODE_CONFIG) as [keyof typeof MODE_CONFIG, typeof MODE_CONFIG[keyof typeof MODE_CONFIG]][]).map(
          ([modeKey, config]) => {
            const stats = data[modeKey];
            if (!stats) return null;

            const { enabled, total_active, active_pairs } = stats;
            ...
            return (
              <div
                ...
                } rounded-lg p-4 ${enabled ? "bg-gray-800/30" : "bg-gray-900/50"}`}
              >
                {/* Header */}
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-xs text-gray-500">
                    ...
                    {slotsAvailable > 0 && (
                      <span className="text-gray-600 ml-1">({slotsAvailable} free)</span>
                    )}
                  </div>
                </div>

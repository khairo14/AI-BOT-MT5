...
class SignalBus:
            ...
            if sig.get("status") == "pending" and sig.get("expires_at"):
                try:
                    ...
                    if now >= exp:
                        ...
                        try:
                            # build normalized identity first
                            account_login = (sig.get("account_login") or sig.get("login") or sig.get("account", {}).get("login", 0))
                            account_mode = (sig.get("execution_mode") or sig.get("account_mode") or "live")
                            signal_id = (sig.get("signal_id") or sig.get("id") or f"{account_login}:{sig.get('symbol')}:{sig.get('strategy')}:{sig.get('direction')}")
                            ...
                            signal_journal.record({
                                ...
                                "trading_type": sig.get("trading_mode", ""),
                                ...
                                "sl": sig.get("sl"),
                                "tp": sig.get("tp"),
                                "timeframe": sig.get("timeframe") or sig.get("tf"),
                                "regime": sig.get("regime"),
                                "rl_state": sig.get("rl_state"),
                                "status": "expired",
    ...
    async def add_signal(self, signal: dict) -> dict:
        ...
        if _dedup_key in self._pending_keys:
            ...
            try:
                # build normalized identity first
                account_login = (signal.get("account_login") or signal.get("login") or signal.get("account", {}).get("login", 0))
                account_mode = (signal.get("execution_mode") or signal.get("account_mode") or "live")
                signal_id = (signal.get("signal_id") or signal.get("id") or f"{account_login}:{signal.get('symbol')}:{signal.get('strategy')}:{signal.get('direction')}")
            ...
            if (
                ...
                try:
                    ...
                    signal_journal.record({
                        ...
                        "account_type": signal.get("account_type", ""),
                        "user_id": signal.get("user_id", "default"),
                        "filters": {"filter": "duplicate_existing_signal"}
                    })
                except Exception:
                    pass
                        ...
                        if open_dir == direction:
                            ...
                            try:
                                ...
                                signal_journal.record({
                                    ...
                                    "account_type": signal.get("account_type", ""),
                                    "user_id": signal.get("user_id", "default"),
                                    "filters": {"filter": "open_position_exists","open_direction": open_dir,"new_direction": direction,"symbol": sym,"trading_type": mode,}
                                })
                            except Exception:
                                pass
                        else:
            ...
            if existing_dir == new_dir:
                if _new_score > existing_score:
                    ...
                    try:
                        ...
                        signal_journal.record({
                            ...
                            "account_type": signal.get("account_type", ""),
                            "user_id": signal.get("user_id", "default"),
                            "filters": {"filter": "existing signal replaced by higher-score signal",}
                        })
                    except Exception:
                        pass
                ...
                try:
                    ...
                    signal_journal.record({
                        ...
                        "account_type": signal.get("account_type", ""),
                        "user_id": signal.get("user_id", "default"),
                        "filters": {"filter": "signal_arbitration",}
                    })
                except Exception:
                    pass
            ...
            if score_gap < _conflict_margin:
                ...
                try:
                    ...
                    signal_journal.record({
                        ...
                        "account_type": signal.get("account_type", ""),
                        "user_id": signal.get("user_id", "default"),
                        "filters": {"filter": "Direction conflict",}
                    })
                except Exception:
                    pass
                return signal
        ...
        if self._order_manager is None or self._client is None:
            ...
            try:
                ...
                signal_journal.record({
                    ...
                    "account_type": signal.get("account_type", ""),
                    "user_id": signal.get("user_id", "default"),
                    "filters": {"filter": "SignalBus not initialised",}
                })
            except Exception:
                pass
                ...
                if not cb_allowed:
                    ...
                    try:
                        ...
                        signal_journal.record({
                            ...
                            "account_type": signal.get("account_type", ""),
                            "user_id": signal.get("user_id", "default"),
                            "filters": {"filter": "circuit breaker blocked",}
                        })
                    except Exception:
                        pass
                    ...
                    if not strat_ok:
                        ...
                        try:
                            ...
                            signal_journal.record({
                                ...
                                "account_type": signal.get("account_type", ""),
                                "user_id": signal.get("user_id", "default"),
                                "filters": {"filter": "per-strategy circuit breaker",}
                            })
                        except Exception:
                            pass
                        return False
                    ...
                    if not allowed:
                        ...
                        try:
                            ...
                            signal_journal.record({
                                ...
                                "account_type": signal.get("account_type", ""),
                                "user_id": signal.get("user_id", "default"),
                                "filters": {"filter": "concurrent limit blocked",}
                            })
                        except Exception:
                            pass
        ...
        try:
                                    ...
                                    if _rv_lot < _min_rv:
                                        ...
                                        try:
                                            ...
                                            signal_journal.record({
                                                ...
                                                "account_type": signal.get("account_type", ""),
                                                "user_id": signal.get("user_id", "default"),
                                                "filters": {"filter": "rl_risk_factor_below_min_lot",}
                                            })
                                        except Exception:
                                            pass
                                        return False
            ...
            if result and result.success:
                ...
                try:
                    ...
                    signal_journal.record({
                        ...
                        "account_type": signal.get("account_type", ""),
                        "user_id": signal.get("user_id", "default"),
                        "filters": {"filter": "order manager failed",}
                    })
                except Exception:
                    pass
            ...
            try:
                ...
                signal_journal.record({
                    ...
                    "account_type": signal.get("account_type", ""),
                    "user_id": signal.get("user_id", "default"),
                    "filters": {"filter": "execution exception",}
                })
            except Exception:
                pass
            return False

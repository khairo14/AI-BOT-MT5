...
# Regime timeframe per strategy.
# Keep trend-following scalpers on stable H1 regime labels; use M15 for
# vwap_reversion so mean-reversion gating reflects intraday structure.
_REGIME_TF: dict[str, str] = {
    "ema_scalp":           "H1",
    "bb_squeeze":          "H1",
    "vwap_reversion":      "M15",
...
class StrategyRunner:
    ...
    def _run_strategy(
        ...
        *,
        ...
        _regime_tf = _REGIME_TF.get(strat_name, _ptf or "H1")
        _regime_df = tf_data.get(_regime_tf, primary_df)
        try:
            from engine.regime_classifier import regime_classifier as _rc
            _regime: str | None = _rc.classify(symbol, _regime_df, timeframe=_regime_tf)
        except Exception:
            _regime = None
